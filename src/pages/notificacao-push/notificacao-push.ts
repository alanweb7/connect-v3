...
export class NotificacaoPushPage {
  ...
  createPusg() {
    ...
    if (!this.pushForm.valid) {
      ...
      this.codeProvider.create_push(this.code, this.model.titulo, this.model.live, this.statuslive, this.model.mensagem, this.token, this.lang)
        .then(
          (result: any) => {
  ...
  changeLive() {
    this.statuslive = this.statuslive ? false : true;
    console.log('Status da Live', this.statuslive);
